...
export async function anchorProofOnSolana(
  ...
  const memoText     = `KARTA:${projectId}:${proofHash}:${co2eAmount}:${Date.now()}`;
...
export async function retireCredits(
  ...
  const MEMO_PROGRAM = new PublicKey('MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr');
  const memoText     = `KARTA:RETIRE:${projectId}:${co2eAmount}:${organisationName}:${certId}`;
