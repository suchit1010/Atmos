...
pub mod atmos_protocol {
    ...
    pub fn mint_carbon_credits(
        ...
        // Rate limiting: Check user's mint count today
        ...
        // ────────────────────────────────────────────────────────
        // CRITICAL: Verify ZK proof (upgraded from placeholder)
        // ────────────────────────────────────────────────────────
        verify_zk_proof(
            &zk_proof,
            &project_id,
        ...
        // Create verification record (deterministic PDA by project_id)
        let verification_record = &mut ctx.accounts.verification_record;
        verification_record.project_id = project_id.clone();
        ...
        verification_record.bump = *ctx.bumps.get("verification_record").unwrap();
    ...
    pub fn retire_carbon_credits(
        ...
        retirement.bump = *ctx.bumps.get("retirement_record").unwrap();
...
pub struct MintCarbonCredits<'info> {
    ...
    pub user_daily_limit: Account<'info, UserDailyLimit>,
